...
proc delete_line {} {
	...
	for {set i $h} {$i > 0} {incr i -1} {
		set blocks_delete [.canvas find overlapping 0 $i $w $i]
		if {[llength $blocks_delete]-5 > $w/$block_size} {
			foreach block_delete $blocks_delete {
				add_score
				.canvas delete $block_delete
			}
		}
	}
}
...
proc create {coords color} {
	...
	foreach coord $coords {
		lappend current_block_name [.canvas create rectangle $coord -fill $color]
	}
	...
	foreach block_coords $current_block {
 	...
 	if $moving {
		...
		foreach block_name $current_block_name {
			.canvas coords $block_name [lindex $current_block $i]
			incr i
		}
 	}
}
